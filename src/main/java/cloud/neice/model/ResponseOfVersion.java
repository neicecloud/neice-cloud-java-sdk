...
public class ResponseOfVersion {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ResponseOfVersion
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResponseOfVersion.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ResponseOfVersion is not found in the empty JSON string", ResponseOfVersion.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ResponseOfVersion.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResponseOfVersion` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ResponseOfVersion.openapiRequiredFields) {
      ...
      if (jsonObj.get("data") != null && !jsonObj.get("data").isJsonNull()) {
        Version.validateJsonElement(jsonObj.get("data"));
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ResponseOfVersion.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ResponseOfVersion' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ResponseOfVersion> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ResponseOfVersion.class));

       return (TypeAdapter<T>) new TypeAdapter<ResponseOfVersion>() {
           @Override
           public void write(JsonWriter out, ResponseOfVersion value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ResponseOfVersion read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of ResponseOfVersion given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of ResponseOfVersion
   * @throws IOException if the JSON string is invalid with respect to ResponseOfVersion
   */
  public static ResponseOfVersion fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ResponseOfVersion.class);
  }

  /**
   * Convert an instance of ResponseOfVersion to an JSON string
   *
   * @return JSON string
   */
